...
class Calculator {
    ...
    compute() {
        ...
        /* Special Variables */
        var x1 = new Big(prev);
        var x2 = new Big(current);
        var x2String = current.toString();
        // alert("x2String: " + x2String);
        /* converts */
        if (isNaN(prev) || isNaN(current)) return
        switch (this.operation) {
            case '+':
                computation = x1.plus(x2).valueOf();
                break
            case '-':
                computation = x1.minus(x2).valueOf();
                break
            case '*':
                computation = x1.times(x2).valueOf();
                break
            case '÷':
                if (x2 == 0){
